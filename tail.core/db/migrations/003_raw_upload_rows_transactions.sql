...
CREATE INDEX IF NOT EXISTS idx_raw_uploads_source_uploaded
  ON raw_uploads(source_id, uploaded_at DESC);
CREATE INDEX IF NOT EXISTS idx_raw_uploads_sync_run
  ON raw_uploads(sync_run_id);
CREATE INDEX IF NOT EXISTS idx_raw_uploads_content_sha
  ON raw_uploads(content_sha256);
...
CREATE INDEX IF NOT EXISTS idx_raw_rows_upload_row_index
  ON raw_rows(upload_id, row_index);
CREATE UNIQUE INDEX IF NOT EXISTS uq_raw_rows_row_sha256
  ON raw_rows(row_sha256);
