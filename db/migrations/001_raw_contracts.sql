...
CREATE INDEX IF NOT EXISTS idx_sync_runs_source
  ON sync_runs (source_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS raw_events (
  ...
  CONSTRAINT raw_events_payload_meta_object_chk CHECK (
    NOT (payload ? '_meta') OR jsonb_typeof(payload -> '_meta') = 'object'
  ),
...
CREATE INDEX IF NOT EXISTS idx_raw_events_run
  ON raw_events (sync_run_id);

CREATE INDEX IF NOT EXISTS idx_raw_events_source
  ON raw_events (source_id, ingested_at DESC);

CREATE INDEX IF NOT EXISTS idx_raw_events_parse_status
  ON raw_events (parse_status);

CREATE UNIQUE INDEX IF NOT EXISTS uq_raw_events_dedupe
